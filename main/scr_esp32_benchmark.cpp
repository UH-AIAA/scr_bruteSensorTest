...
// TODO: this task is an example of reading the BMP
void LSM_task(void *pvParameter)
{
    while(1)                // freertos tasks run in a forever while loop (the RTOS handles switching, dw)
    {
        ...
        {
            taskYIELD();    // this is like ending the function early: the RTOS will try something else and come back
        }

        // Store gyroscope data
        float lsm_gyro_x = gyro.gyro.x;
        float lsm_gyro_y = gyro.gyro.y;
        float lsm_gyro_z = gyro.gyro.z;

        // Store accelerometer data
        float lsm_acc_x = accel.acceleration.x;
        float lsm_acc_y = accel.acceleration.y;
        float lsm_acc_z = accel.acceleration.z;

        // Store temperature data
        float lsm_temp = float(temp.temperature);

        // get our uptime, print to serial monitor
